...
class Point2VecPCP(Point2Vec):
    def __init__(
        ...
        pcp_pos_decoder_depth: int = 2,
        pcp_pos_decoder_dropout: float = 0.0,
        pcp_pos_decoder_attention_dropout: float = 0.05,
        pcp_pos_decoder_drop_path_rate: float = 0.1,
        ...
        if not decoder:
            raise ValueError("Point2VecPCP requires decoder=True.")
        ...
        pos_dpr = [
            ...
            for x in torch.linspace(
                0, pcp_pos_decoder_drop_path_rate, pcp_pos_decoder_depth
            )
        ]
        self.pos_decoder = TransformerEncoder(
            embed_dim=encoder_dim,
            depth=pcp_pos_decoder_depth,
            num_heads=encoder_heads,
            qkv_bias=True,
            drop_rate=pcp_pos_decoder_dropout,
            attn_drop_rate=pcp_pos_decoder_attention_dropout,
            drop_path_rate=pos_dpr,
    ...
    def forward(
        ...
        num_masked = masked_embeddings.shape[1]
        mask_pos_token = self.mask_pos_token.reshape(1, 1, C).expand(B, num_masked, -1)

        pos_decoder_output = self.pos_decoder(
            torch.cat([output_embeddings, masked_embeddings], dim=1),
            torch.cat([visible_pos, mask_pos_token], dim=1),
        ).last_hidden_state
        predicted_masked_pos = self.pred_pos_proj(pos_decoder_output[:, -num_masked:])

        decoder_masked_pos = predicted_masked_pos.detach()
        if not self.hparams.pcp_detach_predicted_pos:  # type: ignore
            decoder_masked_pos = predicted_masked_pos

        decoder_output_tokens = self.decoder(
            torch.cat([output_embeddings, masked_embeddings], dim=1),
            torch.cat([visible_pos, decoder_masked_pos], dim=1),
        ).last_hidden_state

        predictions = self.regressor(
            decoder_output_tokens[:, -num_masked:].reshape(-1, C)
        )
        ...
        match self.hparams.pcp_pos_prediction_loss:  # type: ignore
            case "mse":
                pos_loss = F.mse_loss(predicted_masked_pos, masked_pos.detach())
            case "smooth_l1":
                pos_loss = F.smooth_l1_loss(predicted_masked_pos, masked_pos.detach())
            case _:
                raise ValueError(
